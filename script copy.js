...
const rectFlipper = new Flipper( W/2-170, canvas.height - 100,  20, 330, 15);
...
const ball = new Ball(throwingMechanism.x + throwingMechanism.width / 2, throwingMechanism.y - 120, 10, 1, 2, 'steel');
//const ball = new Ball(W/2-33, 60, 10, 1, 1, 'steel');
...
function handleCollisions() {
  ...
  if (hasCollidedWithWall) {
    console.log('Ball collided with the wall');
  }
  const hasCollidedWithWall2 = ballWallCollision(ball, wall2X, wall2Y, wall2Width, wall2Height);
if (hasCollidedWithWall2) {
  console.log('Ball collided with the second wall');
}
...
if (hasCollidedWithHorizontalWall) {
  console.log('Ball collided with the horizontal wall');
}
...
if (hasCollidedWithHorizontalWall2) {
  console.log('Ball collided with the second horizontal wall');
  // Additional response to collision can go here
}
...
if (!isPause) {
  ...
  // Update and draw each ball
      ...
      if (!ball.onThrowingMechanism) {
        ...
        const speedDampingFactor = 0.7; // Adjust this value as needed
        ...
        if (!ball.inPlay && ballAngle <= arcStartAngle && ballAngle >= arcEndAngle ) {
          ...
          // Apply damping to reduce speed as the ball leaves the arc
          ball.speedX = -3
          ball.speedY = 3
        }
        else {
  ...
  if (ball.correctionVelocity.x !== 0 || ball.correctionVelocity.y !== 0) {
    ...
    // Handle ball and arc segment collision
    ballArcSegmentCollision(ball, arcCenterX, arcCenterY, radius, startAngle, endAngle, lineWidth);
...
if (downKeyIsPressed) {
  ...
  if (throwingMechanism.height > 16) {
    ...
    throwingMechanism.compressionTime += 0.5; // Adjust this value as needed
  }
} else {
